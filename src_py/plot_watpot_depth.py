...
def main():
    ...
    #select soil untill 5 meter depth
    su = su_data[:, 0:ind5]
    ...
    #loop over time, calculate storage and water potential
   ...
   #---------------------------------
    ...
    #######################################################################################
    #make plot
    fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(args.figsize[0], args.figsize[1]), sharex=True, sharey=True)        
    ax = axes.flat

    #new VOM-results
    y = np.insert(-delz_sum[0:ind5],0,0)
    c1 = ax[0].pcolor(watpot_hourly_pd.index, y ,watpot_hourly_pd.values.T,norm=LogNorm(vmin=0.01, vmax=250), vmin=0.01, vmax=250, cmap = 'RdYlGn')
    ...
    ax[0].set_ylabel("Depth (m)", size=20)
    ...
    c2 = ax[1].pcolor(watpot_hourly2015_pd.index, y2015, watpot_hourly2015_pd.values.T, norm=LogNorm(vmin=0.01, vmax=250), vmin=0.01, vmax=250, cmap='RdYlGn'   )
    ax[1].plot([datetime(yearstart,1, 1), datetime( yearend ,12, 31)], [- params2015[5],- params2015[5]], ":", lw=3, color='red', label='rtdepth trees')
    ax[1].plot([datetime(yearstart,1, 1), datetime( yearend ,12, 31)], [- params2015[7],- params2015[7]],":",lw=3, color='orange', label='rtdepth grasses')

    #set colorbar
    cb = fig.colorbar(c2,ax=axes.ravel().tolist())
