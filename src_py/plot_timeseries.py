...
def main():
    ...
    if args.i2015 is not None:
        ...
        for ivar in range(0, len(args.var)):
            ...
            if(var == "ncpt"):
                data2015["ncpt"] = np.nan(len(tmod2015))
            if(var == "ncpg"):
                data2015["ncpg"] = np.nan(len(tmod2015))
    ...
    if args.i2015 is not None:
        for iplot in range(0, len(args.var)):
            ax[iplot, 0].plot(tmod2015, data2015[args.var[iplot]], color='green', label='Schymanski et al. (2015)', zorder=2)
    ...
    for iplot in range(0, len(args.var)):
        ...
        ax[iplot, 0].set_xlim([datetime(yearstart,1, 1), datetime( yearend ,12, 31)]) 
