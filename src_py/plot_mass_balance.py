...
def main():
    ...
    fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(args.figsize[0], 
    ...
    if args.i2015 is not None:
        ...
        WB = np.mean(data2015["rain"]) - np.mean(data2015["evaptot"] ) - np.mean(data2015["spgfcf"] ) - np.mean(data2015["infx"] )

        ax[iplot].plot([-1,20], [WB,WB], color='green', lw=2,label='Schymanski et al. (2015)', zorder=2)
    ...
    for i in range(0, len(args.input)):
        WB = np.mean(vals[i]["rain"]) - np.mean(vals[i]["evaptot"] ) - np.mean(vals[i]["spgfcf"] ) - np.mean(vals[i]["infx"] )
        ax[iplot].bar([i], WB, color="blue", zorder=1)                 

        print(WB)
          
               
        #set axis and ticks   
        ax[iplot].set_ylabel("WB [mm/year]" , size=args.labelsize  )
