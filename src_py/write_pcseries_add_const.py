...
def main():
    ...
    pc_result[fpar.index] = (fpar/0.95) - const_cov
    pc_result[pc_result < 0.0] = 0.0
    ...
    for i in range(0,len(pc_result)):
        if( np.isnan(pc_result[ i ])):
            ...
            if(  any(~(np.isnan(pc_result[ (pc_result.index.month ==month) & (pc_result.index.year == year)] )))):
                pc_result[i] = np.nanmean(pc_result[ (pc_result.index.month ==month) & (pc_result.index.year == year)])
            else:
